...
class d3piechart extends d3chart {
  ...
  initChart() {
    ...
    if (this.cfg.radius && this.cfg.radius.inner) {
      const outRadius = this.cfg.radius.outter
        ? this.cfg.radius.outter
  ...
  setScales() {
    // Set up radius
    this.cfg.radius.outter =
      this.cfg.radius && this.cfg.radius.outter
        ? this.cfg.radius.outter
        : Math.min(this.cfg.width, this.cfg.height) / 2;
    let inRadius =
      this.cfg.radius && this.cfg.radius.inner ? this.cfg.radius.inner : 0;
  ...
  enterElements() {
    ...
    newg
      ...
      .attr('transform', (d) => {
        ...
        pos[0] =
          this.cfg.radius.outter * (this.midAngle(d) < Math.PI ? 1.1 : -1.1);
    ...
    newg
      ...
      .attr('points', (d) => {
        ...
        pos[0] =
          this.cfg.radius.outter *
          0.95 *
          (this.midAngle(d) < Math.PI ? 1.1 : -1.1);
